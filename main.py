...
async def extract_ips_from_vc(chat_id, link=None):
    ...
    try:
        ...
        # 2. Get chat object (this will have access_hash if available)
        ...
        try:
            ...
            if member.status in ["left", "kicked"]:
                return None, "❌ Account is not a member. Add account or use invite link."
        except:
            return None, "❌ Cannot verify membership. Try invite link."

        # 4. Get full chat details to access VC
        # Use high-level method: get chat with full info
        full_chat = await user.get_chat(chat_id)
        # But get_chat doesn't have call, we need raw API but without custom constructors
        # Use the raw method with peer directly
        from pyrogram.raw import functions, types
        peer = await user.resolve_peer(chat_id)
        
        if isinstance(peer, types.InputPeerChannel):
            # Use InputChannel with keyword arguments
