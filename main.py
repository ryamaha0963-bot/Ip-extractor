...
# Enable logging to see errors on Railway console
...
# ---------- 🔥 PRO LEVEL: GET PEER WITHOUT access_hash ERROR ----------
async def get_input_peer_safe(user, chat_id):
    """Brute-force search in dialogs FIRST, then fallback to resolve_peer"""
    try:
        async for dialog in user.get_dialogs():
            if dialog.chat.id == chat_id:
                LOGGER.info(f"✅ Found chat in dialogs: {chat_id}")
                return dialog.peer  # Raw InputPeer with access_hash
    except Exception as e:
        LOGGER.warning(f"Dialog scan error: {e}")

    # Fallback 1: Try resolve_peer
    try:
        peer = await user.resolve_peer(chat_id)
        LOGGER.info(f"✅ Resolved via fallback: {chat_id}")
        return peer
    except Exception as e:
        LOGGER.error(f"Resolve fallback failed: {e}")

    return None  # Not found

# ---------- CORE EXTRACT (100% SOLID) ----------
async def extract_ips_from_vc(chat_id, link=None):
    ...
    try:
        ...
        # 2. 🔥 GET PEER WITHOUT access_hash ERROR
        peer = await get_input_peer_safe(user, chat_id)
        if not peer:
            return None, "❌ Account is NOT a member, or group ID is invalid. Try invite link."

        # 3. Get full chat to access VC call
        ...
        # 4. Join VC (to get connection data)
        try:
            ...
            LOGGER.warning(f"Join error (ignored): {e}")
