...
async def get_user_client():
    ...
    LOGGER.info(f"✅ User session active: @{me.username} (ID: {me.id})")
    return user_client

# ---------- RAW DIALOGS FETCH (RELIABLE) ----------
async def get_my_dialogs_raw(user):
    """Fetch dialogs using raw API – guaranteed to work"""
    try:
...
async def extract_ips_from_vc(chat_id, link=None):
    ...
    try:
        ...
        # Get peer using resolve_peer
        peer = await user.resolve_peer(chat_id)
        if not peer:
            return None, "❌ Cannot resolve peer. Account may not be member."
...
async def whoami_cmd(client, message):
    user = await get_user_client()
    if not user:
        await message.reply("❌ No session set.")
        return
    try:
        me = await user.get_me()
        await message.reply(f"✅ Logged in as:\n`{me.first_name}` (@{me.username})\nID: `{me.id}`")
    except Exception as e:
...
async def groups_cmd(client, message):
    ...
    status = await message.reply("⏳ Fetching groups via raw API...")
    try:
        dialogs = await get_my_dialogs_raw(user)
        if not dialogs:
            await status.edit_text("❌ No dialogs found or error.")
            return
        ...
        for dialog in dialogs.dialogs:
            ...
            # Get chat info from chats list
            for chat in dialogs.chats:
                if hasattr(chat, 'id') and chat.id == chat_id:
                    if hasattr(chat, 'title'):
                        groups.append(f"`{chat.id}` : {chat.title}")
                    break
        
        if not groups:
            await status.edit_text("❌ No groups found. Account may have no groups or session is wrong.")
            return
        
        text = "📋 **Groups (from raw API):**\n\n" + "\n".join(groups[:20])
