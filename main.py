...
# ---------- CORE EXTRACT (NO InputGroupCall ERROR) ----------
    ...
    try:
        ...
        if link:
            ...
            try:
                ...
                return None, "❌ Account is NOT a member. Use invite link or add account."

        # 2. Get full chat to access VC call
        try:
            ...
            return None, f"Failed to fetch chat: {e}"

        call = getattr(full.full_chat, "call", None)
        if not call:
            return None, "📢 No active Voice Chat."

        # 3. Join VC (using raw call but with correct InputGroupCall)
        try:
            me = await user.resolve_peer('me')
            # Get call params
            # NOTE: Pyrogram's InputGroupCall expects only `id` sometimes, but we pass both.
            # Using raw types from pyrogram.raw.types
