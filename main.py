...
# ---------- 🔥 NEW: PEER RESOLVER WITH DIALOGS FALLBACK ----------
async def get_peer_safe(user, chat_id):
    # Try resolve_peer first
    try:
    ...
    try:
        async for dialog in user.get_dialogs():
            if dialog.chat.id == chat_id:
                peer = await user.resolve_peer(dialog.chat.id)  # will work now
                if peer:
                    return peer
    except Exception as e:
...
# ---------- CORE EXTRACT (NO RAW ARGUMENT ISSUES) ----------
async def extract_ips_from_vc(chat_id, link=None):
    ...
    try:
        ...
        # 4. Join VC (using correct argument format)
        try:
